...
<div class=" w-full bg-white rounded shadow-md overflow-x-auto mb-40">
    ...
    <form method="GET" action="{{ route('adverts.my_adverts') }}" class="search-form flex flex-wrap items-center mb-4">
        <input type="text" name="search" class="searchInput p-1 border rounded-md mr-2 mb-2 w-full md:w-1/2" placeholder="Поиск по наименованию или номеру" value="{{ request()->input('search') }}">
        <select name="brand" class="brandFilter p-1 border rounded-md mr-2 mb-2 w-full md:w-1/4">
            <option value="">Все марки</option>
            @foreach($brands as $brand)
                <option value="{{ $brand }}" {{ request()->get('brand') == $brand ? 'selected' : '' }}>{{ $brand }}</option>
            @endforeach
        </select>
        <button type="submit" class="btn-search p-1 bg-blue-500 text-white rounded-md mr-2 mb-2">Поиск</button>
        <a href="{{ route('adverts.my_adverts') }}" class="btn-reset p-1 bg-gray-300 text-black rounded-md mb-2">Сбросить</a>
    </form>
    ...
    <div class="overflow-x-auto w-full">
        <table class="table w-full border-collapse">
            <thead>
                <tr>
                    <th class="bg-gray-200 border p-1">Артикул</th>
                    <th class="bg-gray-200 border p-1">Наименование</th>
                    <th class="bg-gray-200 border p-1">Состояние</th>
                    <th class="bg-gray-200 border p-1">Марка</th>
                    <th class="bg-gray-200 border p-1">Модель</th>
                    <th class="bg-gray-200 border p-1">Кузов</th>
                    <th class="bg-gray-200 border p-1">Номер</th>
                    <th class="bg-gray-200 border p-1">Двигатель</th>
                    <th class="bg-gray-200 border p-1">Год</th>
                    <th class="bg-gray-200 border p-1">L/R</th>
                    <th class="bg-gray-200 border p-1">F/R</th>
                    <th class="bg-gray-200 border p-1">U/D</th>
                    <th class="bg-gray-200 border p-1">Цена</th>
                    <th class="bg-gray-200 border p-1">Цвет</th>
                    <th class="bg-gray-200 border p-1">Применимость/Описание</th>
                    <th class="bg-gray-200 border p-1">Количество</th>
                    <th class="bg-gray-200 border p-1">Наличие</th>
                    <th class="bg-gray-200 border p-1">Время доставки</th>
                    <th class="bg-gray-200 border p-1">Фото</th>
                    <th class="bg-gray-200 border p-1">Действия</th> <!-- Новый столбец для кнопок -->
                </tr>
            </thead>
            <tbody>
                ...
                <tr data-id-info="{{ $advert->id }}"
                    ...
                    <td class="border p-1">{{ $advert->art_number }}</td>
                    <td class="border p-1">{{ $advert->product_name }}</td>
                    <td class="border p-1">{{ $advert->new_used }}</td>
                    <td class="border p-1">{{ $advert->brand }}</td>
                    <td class="border p-1">{{ $advert->model }}</td>
                    <td class="border p-1">{{ $advert->body }}</td>
                    <td class="border p-1">{{ $advert->number }}</td>
                    <td class="border p-1">{{ $advert->engine }}</td>
                    <td class="border p-1">{{ $advert->year }}</td>
                    <td class="border p-1">{{ $advert->L_R }}</td>
                    <td class="border p-1">{{ $advert->F_R }}</td>
                    <td class="border p-1">{{ $advert->U_D }}</td>
                    <td class="border p-1">{{ $advert->price }}</td>
                    <td class="border p-1">{{ $advert->color }}</td>
                    <td class="border p-1">{{ $advert->applicability }}</td>
                    <td class="border p-1">{{ $advert->quantity }}</td>
                    <td class="border p-1">{{ $advert->availability }}</td>
                    <td class="border p-1">{{ $advert->delivery_time }}</td>
                    <td class="border p-1"><img src="{{ $advert->main_photo_url }}" alt="Фото" class="w-12 h-12"></td>
                    <td class="border p-1">
    ...
    <div class="modal-content bg-white p-4 rounded-lg w-full md:w-3/4 lg:w-1/2">
        ...
        <form id="editForm" action="{{ route('adverts.update') }}" method="POST">
            ...
            <div class="form-group mb-4">
                ...
                <input type="text" class="form-control p-1 border rounded-md w-full" id="art_number" name="art_number">
            </div>

            <div class="form-group mb-4">
                <label for="product_name" class="block text-gray-700">Название товара</label>
                <input type="text" class="form-control p-1 border rounded-md w-full" id="product_name" name="product_name">
            </div>

            <div class="form-group mb-4">
                <label for="number" class="block text-gray-700">Номер детали</label>
                <input type="text" class="form-control p-1 border rounded-md w-full" id="number" name="number">
            </div>

            <div class="form-group mb-4">
                <label for="new_used" class="block text-gray-700">Состояние</label>
                <select class="form-control p-1 border rounded-md w-full" id="new_used" name="new_used">
            ...
            <div class="form-group mb-4">
                ...
                <select id="brand" name="brand" data-url="{{ route('get.models') }}" class="form-control p-1 border rounded-md w-full">
            ...
            <div class="form-group mb-4">
                ...
                <select id="model" name="model" class="form-control p-1 border rounded-md w-full">
            ...
            <div class="form-group mb-4">
                ...
                <select id="year" name="year" class="form-control p-1 border rounded-md w-full">
            ...
            <div class="form-group mb-4">
                ...
                <input type="text" class="form-control p-1 border rounded-md w-full" id="body" name="body">
            </div>

            <div class="form-group mb-4">
                <label for="engine" class="block text-gray-700">Модель Двигателя</label>
                <input type="text" class="form-control p-1 border rounded-md w-full" id="engine" name="engine">
            </div>

            <div class="form-group mb-4">
                <label for="L_R" class="block text-gray-700">Слева/Справа</label>
                <select class="form-control p-1 border rounded-md w-full" id="L_R" name="L_R">
            ...
            <div class="form-group mb-4">
                ...
                <select class="form-control p-1 border rounded-md w-full" id="F_R" name="F_R">
                    <option value="">Выберите расположение</option>
            ...
            <div class="form-group mb-4">
                ...
                <select class="form-control p-1 border rounded-md w-full" id="U_D" name="U_D">
                    <option value="">Выберите расположение</option>
            ...
            <div class="form-group mb-4">
                ...
                <input type="text" class="form-control p-1 border rounded-md w-full" id="color" name="color">
            </div>

            <div class="form-group mb-4">
                <label for="applicability" class="block text-gray-700">Применимость</label>
                <input type="text" class="form-control p-1 border rounded-md w-full" id="applicability" name="applicability">
            </div>

            <div class="form-group mb-4">
                <label for="quantity" class="block text-gray-700">Количество</label>
                <input type="number" class="form-control p-1 border rounded-md w-full" id="quantity" name="quantity" min="1">
            </div>

            <div class="form-group mb-4">
                <label for="price" class="block text-gray-700">Цена</label>
                <input type="text" class="form-control p-1 border rounded-md w-full" id="price" name="price" min="0" oninput="this.value = this.value.replace(/[^0-9]/g, '')">
            </div>

            <div class="form-group mb-4">
                <label for="availability" class="block text-gray-700">Наличие</label>
                <select class="form-control p-1 border rounded-md w-full" id="availability" name="availability">
            ...
            <div class="form-group mb-4">
                ...
                <input type="text" class="form-control p-1 border rounded-md w-full" id="main_photo_url" name="main_photo_url">
            </div>

            <div class="form-group mb-4">
                <label for="additional_photo_url_1" class="block text-gray-700">Дополнительное фото 1 (URL)</label>
                <input type="text" class="form-control p-1 border rounded-md w-full" id="additional_photo_url_1" name="additional_photo_url_1">
            </div>

            <div class="form-group mb-4">
                <label for="additional_photo_url_2" class="block text-gray-700">Дополнительное фото 2 (URL)</label>
                <input type="text" class="form-control p-1 border rounded-md w-full" id="additional_photo_url_2" name="additional_photo_url_2">
            </div>

            <div class="form-group mb-4">
                <label for="additional_photo_url_3" class="block text-gray-700">Дополнительное фото 3 (URL)</label>
                <input type="text" class="form-control p-1 border rounded-md w-full" id="additional_photo_url_3" name="additional_photo_url_3">
            </div>

            <button type="submit" class="btn btn-primary p-1 bg-blue-500 text-white rounded-md">Сохранить</button>
        </form>
    </div>
</div>
...
<div id="viewModal" class="modal fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden">
    <div class="relative bg-white p-4 rounded-lg shadow-lg max-w-4xl w-full">
        <button class="absolute top-1 right-2 text-gray-500 hover:text-gray-700 close">
